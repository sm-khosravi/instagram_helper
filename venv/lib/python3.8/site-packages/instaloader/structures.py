...
class Post:
    ...
    # added propeerties
    @property
    def location_2(self) -> Dict[str, Any]:
        self._obtain_metadata()
        assert self._full_metadata_dict is not None
        return self._full_metadata_dict['location']

    @property
    def display_resources(self) -> Dict[str, Any]:
        self._obtain_metadata()
        assert self._full_metadata_dict is not None
        return self._full_metadata_dict['display_resources']

    def graph_sidecar_children(self) -> Dict[str, Any]:
        self._obtain_metadata()
        assert self._full_metadata_dict is not None
        edge_sidecar_to_children = self._full_metadata_dict.get('edge_sidecar_to_children', None)
        edges = edge_sidecar_to_children.get('edges', None) if edge_sidecar_to_children else None
        return edges
...
class Profile:
    ...
    def get_igtv_videos(self) -> NodeIterator[Post]:
        """Retrieve all IGTV videos.

        :rtype: NodeIterator[Post]

        .. versionadded:: 4.3"""
        # self._obtain_metadata()
        edge_felix_video_timeline = self._node['edge_felix_video_timeline']
        edges = edge_felix_video_timeline['edges']
        list_video_url = []
        for edge in edges:
            node = edge['node']
            video_url = node['video_url']
            list_video_url.append(video_url)
